...
dotnet test $pathWebAPITests/PoC.SharpDiff.WebAPI.Tests.csproj --configuration Release --logger "trx;LogFileName=TestResults.trx" --logger "xunit;LogFileName=TestResults.xml" --results-directory $pathWebAPITests/BuildReports/UnitTests /p:CollectCoverage=true /p:CoverletOutput=$pathWebAPITests/BuildReports/Coverage/ "/p:CoverletOutputFormat=\"cobertura,opencover\"" /p:Exclude="[xunit.*]*"
dotnet test $pathIntegrationTests/PoC.SharpDiff.Tests.csproj --configuration Release --logger "trx;LogFileName=TestResults.trx" --logger "xunit;LogFileName=TestResults.xml" --results-directory $pathIntegrationTests/BuildReports/UnitTests /p:CollectCoverage=true /p:CoverletOutput=$pathIntegrationTests/BuildReports/Coverage/ "/p:CoverletOutputFormat=\"cobertura,opencover\"" /p:Exclude="[xunit.*]*"
