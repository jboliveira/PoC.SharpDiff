...
dotnet test $pathIntegrationTests/PoC.SharpDiff.Tests.csproj -c $netConfiguration --logger "trx;LogFileName=TestResults.trx" --logger "xunit;LogFileName=TestResults.xml" --results-directory $pathIntegrationTests/BuildReports/UnitTests /p:CollectCoverage=true /p:CoverletOutput=$pathIntegrationTests/BuildReports/Coverage/ "/p:CoverletOutputFormat=\"cobertura,opencover\"" /p:Exclude="[xunit.*]*"
